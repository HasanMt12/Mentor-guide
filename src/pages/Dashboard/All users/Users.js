    ...
    const handleMakeVerified = id => {
        ...
        .then(data => {
            ...
            if(data.modifiedCount > 0 ){
                toast.success('make mentor verified successfully')
                refetch();
            }
        })
    }
    ...
    <tbody>
      {
        users.map((user, i) =><tr key={user._id}>
            ...
            <td>{user.email}</td>
            <td>{user.role}</td>
           <td>{ user?.verification !== 'verify' ? <button onClick={()=> handleMakeVerified(user._id)} className="btn btn-xs  text-gray-300 ">make verified</button>
           :<p className='text-green-500 '> verified Mentor</p>} </td>
